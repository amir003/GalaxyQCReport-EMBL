		    ...
		    if(tableEl.length>0 ){
		    	if(tableEl.get(0).tagName=='TABLE'){
			    	...
			    	if(controls !=''){
						...
						//controlsEl.find("a[data-attc-controls-addCreateOptions]").on("click", function(e){ // Commented in order to always have the edition options
							//e.preventDefault();
			    			...
			    			tableEl.find('thead th.drawable').each(function(index) { // '.drawable' added to select only theses columns
								...
								if(isNaN(items) || items==0){
									inputType='radio" disabled="disabled" style="display:none" ';// ADD lock this checkbox 
									inputColType='desc';
								}
								*/
								//force back to radio if pie chart
								if(tableEl.attr('data-attc-type')=='pie'){
									  if(isNaN(items)){ //|| items==0){ || tableEl.find('thead th span').text()==="Sample"){
			                              inputType='radio" ';// disabled="disabled" style="display:none" ';
			                          }
			                          else{
			                              inputType='radio"';
			                          }
